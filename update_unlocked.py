...
def modify_file():
    path = 'src/app/home/unlockeddocuments/UnlockedDocsGrid.tsx'
    with open(path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Find the Desktop Grid map
    d_grid = content.find('{/* Dossier Grid */}')
    c_start = content.find(') : farmlands.map((card, idx) => (', d_grid)
    c_end = content.find('))}</div>', c_start)
    
    if c_start == -1 or c_end == -1:
        print("Could not find desktop map")
        return
        
    new_card_code = ''') : farmlands.map((card, idx) => (
                    <div
    ...
    content = content[:c_start] + new_card_code + content[c_end:]
...
'''
    # We must insert the inline grid code inside the return statement right before the conversion banner of the desktop view
    first_banner = content.find('{/* Conversion Banner */}')
    desktop_banner = content.find('{/* Conversion Banner */}', first_banner + 1)
    
    if desktop_banner != -1:
        content = content[:desktop_banner] + inline_grid_code + '\n' + content[desktop_banner:]
    else:
        print("Could not find second conversion banner")
        return

    m_start = content.find('{/* ─── SHARED DOSSIER MODAL ─── */}')
    if m_start != -1:
        m_end = content.find('</div>\n  );\n}', m_start)
        if m_end != -1:
