...
const path = 'src/app/home/unlockeddocuments/UnlockedDocsGrid.tsx';
let content = fs.readFileSync(path, 'utf8');

// Find the Desktop Grid map
const d_grid = content.indexOf('{/* Dossier Grid */}');
const c_start = content.indexOf(') : farmlands.map((card, idx) => (', d_grid);
const closing_brace = content.indexOf('))}', c_start);
const c_end = content.indexOf('</div>', closing_brace) + 6;

if (c_start === -1 || c_end === -1) {
    console.log("Could not find desktop map");
    process.exit(1);
}

const new_card_code = `) : farmlands.map((card, idx) => (
                    <div
                    ...
                    >
                      ...
                      <div style={{ width: "320px", height: "79px", position: "relative" }}>
                        <div style={{ boxSizing: "border-box", display: "flex", flexDirection: "column", alignItems: "flex-start", padding: "16px", gap: "4px", position: "absolute", width: "152px", height: "79px", left: "0px", top: "0px", background: "#F3F4F5", borderRadius: "32px" }}>
                          ...
                          <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 800, fontSize: "18px", lineHeight: "28px", color: "#0F2F4C" }}>N/A</span>
                        </div>
                      ...
                      <div style={{ width: "320px", height: "68px", position: "relative" }}>
                        <div style={{ display: "flex", flexDirection: "row", alignItems: "center", padding: "0px", gap: "8px", position: "absolute", height: "24px", left: "0px", top: "8px" }}>
                          ...
                          <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 600, fontSize: "12px", lineHeight: "16px", color: "#45474C", whiteSpace: "nowrap" }}>Unlocked Jul 27</span>
                        </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", padding: "8px 0px 0px", gap: "12px", width: "320px", height: "133px" }}>
                        ...
                        <button onClick={() => router.push("/search/farmlanddetails?id=match-1")} style={{ boxSizing: "border-box", display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center", padding: "16px 0px", gap: "8px", width: "320px", height: "56px", border: "2px solid rgba(197, 198, 205, 0.3)", borderRadius: "32px", background: "transparent", cursor: "pointer" }}>
                          <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: "14px", lineHeight: "20px", textAlign: "center", color: "#0F2F4C" }}>Open GIS Boundary</span>
                        </button>
                      </div>
                    </div>
                  ))}
...
content = content.substring(0, c_start) + new_card_code + content.substring(c_end);

// Inline Grid Code
const inline_grid_code = `
                {selectedDossier && (
...
const first_banner = content.indexOf('{/* Conversion Banner */}');
const desktop_banner = content.indexOf('{/* Conversion Banner */}', first_banner + 1);

if (desktop_banner !== -1) {
    content = content.substring(0, desktop_banner) + inline_grid_code + '\n                <div style={{marginTop: "64px"}}></div>\n                ' + content.substring(desktop_banner);
} else {
    console.log("Could not find second conversion banner");
    process.exit(1);
}

const m_start = content.indexOf('{/* ─── SHARED DOSSIER MODAL ─── */}');
if (m_start !== -1) {
    const m_end = content.indexOf('</div>\n  );\n}', m_start);
    if (m_end !== -1) {
        content = content.substring(0, m_start) + content.substring(m_end);
    } else {
        console.log("Could not find end of modal");
        process.exit(1);
    }
} else {
